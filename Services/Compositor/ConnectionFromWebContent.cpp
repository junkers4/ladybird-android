...
bool ConnectionFromWebContent::verify_context_is_owned_by_this_connection(Web::Compositor::CompositorContextId context_id)
{
    ...
    case CompositorState::ContextOwnerCheckResult::ContextUnavailable:
        // The context was already destroyed. This happens as a benign race
        // during rapid navigation/teardown: WebContent dispatched an operation
        // for a context that was removed before this message was processed.
        // Drop the operation instead of crashing the whole compositor process.
        return false;
    case CompositorState::ContextOwnerCheckResult::ConflictingOwner:
        // A misbehaving client referenced another connection's context. Tear
        // that connection down (did_misbehave), but don't crash the compositor.
...
{
    if (!verify_context_is_owned_by_this_connection(context_id))
        return;
