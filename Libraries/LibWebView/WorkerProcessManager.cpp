...
Web::HTML::WorkerAgentId WorkerProcessManager::start_worker_agent(Owner owner, Web::HTML::WorkerAgentStartRequest request)
{
    ...
    // AD-HOC: Worker process launching may not be supported on every platform (Android binds
    //         helper processes as services and does not implement this path yet). Fail the
    //         worker gracefully instead of crashing the UI process.
