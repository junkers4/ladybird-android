...
{
#if defined(AK_OS_ANDROID)
    // FIXME: Bind the WebWorker Android service here (socketpair + JNI) and wrap our end
    //        in a WebWorkerClient, mirroring how WebContent is bound. Until then, workers
    //        are unavailable on Android; WorkerProcessManager fails the worker gracefully.
    (void)type;
    (void)agent_id;
    return Error::from_string_literal("WebWorker processes are not yet supported on Android");
#else
    auto const& browser_options = WebView::Application::browser_options();
    auto const& web_content_options = WebView::Application::web_content_options();
